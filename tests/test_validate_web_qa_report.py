...
class ValidateWebQaReportTests(unittest.TestCase):
    ...
    def test_readme_mentions_governance_sandbox_report_output_directory_aliases_note(self) -> None:
        readme = (Path(__file__).resolve().parents[1] / "README.md").read_text(encoding="utf-8")

        self.assertIn("docs/GOVERNANCE_SANDBOX_REPORT_OUTPUT_DIRECTORY_ALIASES_NOTE.md", readme)
        self.assertTrue(
            (Path(__file__).resolve().parents[1] / "docs" / "GOVERNANCE_SANDBOX_REPORT_OUTPUT_DIRECTORY_ALIASES_NOTE.md").exists()
        )

    def test_readme_mentions_governance_sandbox_report_output_name_alias_note(self) -> None:
        readme = (Path(__file__).resolve().parents[1] / "README.md").read_text(encoding="utf-8")

        self.assertIn("docs/GOVERNANCE_SANDBOX_REPORT_OUTPUT_NAME_ALIAS_NOTE.md", readme)
        self.assertTrue(
            (Path(__file__).resolve().parents[1] / "docs" / "GOVERNANCE_SANDBOX_REPORT_OUTPUT_NAME_ALIAS_NOTE.md").exists()
        )
    ...
    def test_readme_mentions_governance_sandbox_web_demo_ui_ux_playwright_handoff(self) -> None:
        readme = (Path(__file__).resolve().parents[1] / "README.md").read_text(encoding="utf-8")

        self.assertIn("docs/GOVERNANCE_SANDBOX_WEB_DEMO_UI_UX_PLAYWRIGHT_HANDOFF.md", readme)
        self.assertTrue(
    ...
    def test_readme_mentions_governance_sandbox_scenario_inputs_report_note(self) -> None:
        ...
        self.assertTrue(
            (Path(__file__).resolve().parents[1] / "docs" / "GOVERNANCE_SANDBOX_SCENARIO_INPUTS_REPORT_NOTE.md").exists()
        )
    ...
    def test_report_metadata_tracks_missing_recovery_owners_alongside_owned_failures(self) -> None:
        report = """# Sample\n\n## Scope\n- URL: `https://example.test/login`\n- Viewport: `1366x768`\n- Test account: `qa.user@example.test`\n\n## 2) Checklist execution summary\n- Functional checks (3/5 pass)\n  - F1: PASS\n  - F2: FAIL\n    - Expected: Inline selector resolves after submit\n    - Observed: Locator never resolved\n    - First failure timestamp: 2026-03-07T04:10:00Z\n    - Retry: FAIL\n    - Failure classification: selector\n    - Recovery owner: qa-ui\n    - Evidence: `artifacts/f2-failure.png`\n  - F3: FAIL\n    - Expected: Spinner clears within 2 seconds\n    - Observed: Spinner persisted for 10 seconds\n    - First failure timestamp: 2026-03-07T04:12:00Z\n    - Retry: FAIL\n    - Failure classification: runtime\n    - Evidence: `artifacts/f3-failure.png`\n  - F4: PASS\n  - F5: PASS\n- Visual checks (3/3 pass)\n  - V1: PASS `shots/v1.png`\n  - V2: PASS `shots/v2.png`\n  - V3: PASS `shots/v3.png`\n- Off-happy-path checks (2/2 pass)\n  - O1: PASS\n  - O2: PASS\n\n## 3) Execution log\n- F2 checkpoint: FAIL - selector missing after submit\n- F3 checkpoint: FAIL - spinner never cleared\n\n## 4) Signoff\n- Regressions: 2\n- Merge recommendation: **BLOCK**\n- Replay readiness: **BLOCKED**\n- Next action: Investigate F2 first, then rerun F3 with fresh artifacts\n"""
        metadata = _build_report_metadata(report)

        self.assertEqual(metadata["failed_check_recovery_owners"], {"F2": "qa-ui"})
        self.assertEqual(metadata["failed_check_recovery_owner_count"], 1)
        self.assertEqual(metadata["failed_check_recovery_owner_coverage_rate"], 0.5)
        self.assertEqual(metadata["missing_failed_check_recovery_owner_ids"], ["F3"])
        self.assertEqual(metadata["missing_failed_check_recovery_owner_count"], 1)
        self.assertEqual(metadata["next_action_failed_check_recovery_owners"], {"F2": "qa-ui"})
        self.assertEqual(metadata["next_action_failed_check_ids_by_recovery_owner"], {"qa-ui": ["F2"]})
        self.assertEqual(metadata["unresolved_failed_check_recovery_owners"], {})

    def test_report_metadata_summarizes_next_action_replay_support_level(self) -> None:
        report = FAILED_REPORT_WITH_RECOVERY + (
            "- Next action: Re-run F2 on ref=login.submit after capturing `artifacts/f2-rerun.png` and `artifacts/f2-rerun.log`\n"
        )
        metadata = _build_report_metadata(report)

        self.assertEqual(metadata["next_action_target_refs"], ["login.submit"])
        self.assertEqual(
            metadata["next_action_artifact_refs"],
            ["artifacts/f2-rerun.png", "artifacts/f2-rerun.log"],
        )
        self.assertTrue(metadata["next_action_has_target_refs"])
        self.assertTrue(metadata["next_action_has_artifact_refs"])
    ...
    def test_report_metadata_blocks_ready_signoff_when_checkpoint_refs_are_missing(self) -> None:
        ...
        self.assertEqual(
            metadata["effective_replay_readiness_added_blocker_keys_by_section"],
    ...
    def test_readme_links_report_artifact_paths_note(self) -> None:
        repo_root = Path(__file__).resolve().parents[1]
        readme = (repo_root / "README.md").read_text(encoding="utf-8")

        self.assertIn("docs/GOVERNANCE_SANDBOX_REPORT_ARTIFACT_PATHS.md", readme)
        self.assertTrue((repo_root / "docs" / "GOVERNANCE_SANDBOX_REPORT_ARTIFACT_PATHS.md").exists())
