...
If you need the matching PM note for wrapper-shaped scenario files that still carry `stakeholder_types`, open `docs/OPENCLAW_PM_REPO45_STAKEHOLDER_TYPES_WRAPPER_NOTE.md`.
...
If you need a compact PM note for replaying one governance-sandbox scenario through `proposal_input_markdown_path` while keeping the same markdown/html/json report proof lane, open `docs/OPENCLAW_PM_REPO45_PROPOSAL_INPUT_MARKDOWN_PATH_NOTE.md`.
...
If you need a compact PM reminder that the short five-repo report still must keep repo 4 and repo 5 visible on every pass, open `docs/OPENCLAW_PM_FIVE_REPO_SHORT_REPORT_FOUR_FIVE_GATE.md`.
...
If you need a compact PM note for keeping the repo-4/repo-5 phase-one pair anchored to one scenario file and one regenerated report bundle before the five-line status handoff, open `docs/OPENCLAW_PM_REPO45_PHASE_ONE_REPORT_BUNDLE_NOTE.md`.
...
If you need a compact PM cue that the same repo-4/repo-5 pass should name the exact scenario file plus report bundle validation command before push, open `docs/OPENCLAW_PM_REPO45_SCENARIO_REPORT_COMMAND_NOTE.md`.
If you need a compact PM note for keeping the repo-4/repo-5 phase-one pair tied to one validation-first command before the short report, open `docs/OPENCLAW_PM_REPO45_PHASE_ONE_VALIDATION_COMMAND_NOTE.md`.
...
If you need a compact PM reminder that the first web-demo lane should inherit UI/UX and Playwright discipline only after the scenario-file + report bundle stays green, open `docs/GOVERNANCE_SANDBOX_WEB_DEMO_UI_PLAYWRIGHT_PHASE_NOTE.md`.
If you need the matching repo-4/repo-5 handoff that keeps preset/result-card scope behind the same UI/UX-first and Playwright-style phase order, open `docs/OPENCLAW_PM_REPO45_UI_PLAYWRIGHT_PHASE_NOTE.md`.
...
If you need a compact PM note that keeps the future web-demo/result-card lane queued strictly after the scenario-file and report-bundle proof loop, open `docs/OPENCLAW_PM_GOVERNANCE_WEB_DEMO_PHASE_QUEUE_NOTE.md`.
...
If you need the matching PM note for dossier-style exports that arrive as `scenario_dossier` or `scenario_dossier_bundle`, open `docs/GOVERNANCE_SANDBOX_SCENARIO_DOSSIER_BUNDLE_NOTE.md`.
...
If you need the matching repo-4/repo-5 phase-one note for loading that long proposal copy through `proposal_copy_file`, open `docs/OPENCLAW_PM_REPO45_PROPOSAL_COPY_FILE_NOTE.md`.
...
If you need a compact PM cue for keeping one imported scenario tied to one generated report basename before wider demo work, open `docs/GOVERNANCE_SANDBOX_REPORT_BASENAME_ALIAS_CUE.md`.
...
If you need a quick note for `report.output_name`, `report.report_name`, or top-level `report_file_stem` report-file naming, open `docs/GOVERNANCE_SANDBOX_REPORT_FILE_ALIAS_NOTE.md`.
...
If you need the shortest stdin-first reminder that the same JSON/Markdown/HTML report bundle still applies without a scenario file on disk, open `docs/GOVERNANCE_SANDBOX_SCENARIO_REPORT_STDIN_BUNDLE_NOTE.md`.
...
If you need a web UI lane that keeps UI/UX-first structure and Playwright replay discipline aligned for repo 5, open `docs/GOVERNANCE_SANDBOX_WEB_UI_UX_PLAYWRIGHT_BRIDGE_NOTE.md`.
...
If you need a one-line PM cue that keeps scenario-file input tied to a single visible report-output status check, open `docs/GOVERNANCE_SANDBOX_SCENARIO_REPORT_OUTPUT_STATUS_NOTE.md`.
...
If you need the matching repo-4/repo-5 reminder that the same phase-one delivery order still ends at validate-before-push, open `docs/OPENCLAW_PM_REPO45_PHASE_ONE_VALIDATE_THEN_PUSH_NOTE.md`.
...
If you need a compact PM reminder to keep repo-1 handoff wording tied to the same short five-line report format used across the active repos, open `docs/OPENCLAW_PM_FIVE_REPO_HANDOFF_LINE_NOTE.md`.
...
If you need the matching PM note for scenario bundles that load proposal copy from a referenced markdown/text file before the same report-bundle replay, open `docs/GOVERNANCE_SANDBOX_PROPOSAL_FILE_ALIAS_NOTE.md`.
...
If you need the matching PM note for `scenario_rehearsal_pack` wrappers before widening scenario-file reuse, open `docs/GOVERNANCE_SANDBOX_REHEARSAL_PACK_WRAPPER_NOTE.md`.
...
If you need a compact note for the top-level `artifacts_root` alias before widening scenario-to-report wiring, open `docs/GOVERNANCE_SANDBOX_REPORT_ARTIFACTS_ROOT_ALIAS_NOTE.md`.
...
If you need a compact PM reminder to keep repo 5 scenario/report-first while repo 4 still lands one validated preset slice, open `docs/OPENCLAW_PM_REPO45_PHASE_ONE_PRIORITY_NOTE.md`.
...
If you need a compact PM note for keeping download-oriented governance-sandbox bundles compatible with `report.outputs.downloads` and top-level `report_downloads`, open `docs/OPENCLAW_PM_REPO45_REPORT_DOWNLOADS_ALIAS_NOTE.md`.
If you need a compact PM cue for the same repo-4/repo-5 phase-one pair when governance artifact paths are surfaced as `report.outputs.files`, open `docs/OPENCLAW_PM_REPO45_REPORT_OUTPUT_FILES_ALIAS_NOTE.md`.
