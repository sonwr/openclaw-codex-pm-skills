...
- Deterministic replay validation contract: use `--strict-plus --require-qa-inventory-check-refs --json-out artifacts/validation.json` so CI stores replay-ready metadata (`active_profile_preset`, 5/3/2 counts, QA inventory check refs, unresolved failed-check coverage, deduplicated next-action failed-check refs, and checkpoint target/artifact refs for replay triage) as a machine-readable artifact
