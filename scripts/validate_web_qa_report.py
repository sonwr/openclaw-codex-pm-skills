...
def _group_check_ids_by_recovery_owner(check_ids: list[str], recovery_owners: dict[str, str]) -> dict[str, list[str]]:
    grouped: dict[str, list[str]] = {}
    for check_id in check_ids:
        owner = recovery_owners.get(check_id)
        if owner is None:
            continue
        grouped.setdefault(owner, []).append(check_id)
    return grouped
...
def _build_report_metadata(text: str) -> dict[str, object]:
    ...
    effective_replay_readiness_hotspot_checkpoint_ids = [
        checkpoint_id
        for section in hotspot_sections
    ...
    effective_replay_readiness_hotspot_primary_blocker_checkpoint_share = (
        round(
            effective_replay_readiness_hotspot_primary_blocker_checkpoint_count
            / effective_replay_readiness_hotspot_checkpoint_count,
            4,
        )
        if effective_replay_readiness_hotspot_checkpoint_count
        else 0.0
    )
    effective_replay_readiness_hotspot_next_step_by_blocker_key = {
        blocker_key: (
            f"Repair `{blocker_key}` across hotspot checkpoints: " + ", ".join(checkpoint_ids)
            if checkpoint_ids
            else None
        )
        for blocker_key, checkpoint_ids in effective_replay_readiness_hotspot_checkpoint_ids_by_blocker_key.items()
    ...
    effective_replay_readiness_hotspot_next_step_by_section = {
        section: (
            "Repair hotspot section `"
            + section
            + "` blocker(s) "
            + ", ".join(effective_replay_readiness_blocker_keys_by_section[section])
            + " across checkpoints: "
            + ", ".join(checkpoint_ids_by_section[section])
        )
        for section in hotspot_sections
        if effective_replay_readiness_blocker_keys_by_section[section] and checkpoint_ids_by_section[section]
    }
    effective_replay_readiness_hotspot_primary_blocker_key_by_section = {
        section: next(
            (
                blocker_key
                for blocker_key in effective_replay_readiness_blocker_keys_by_section[section]
                if blocker_key in effective_replay_readiness_hotspot_checkpoint_ids_by_blocker_key
            ),
            None,
        )
        for section in hotspot_sections
    ...
    effective_replay_readiness_hotspot_primary_blocker_checkpoint_ids_by_section = {
        section: [
            checkpoint_id
            for checkpoint_id in checkpoint_ids_by_section[section]
            if checkpoint_id in effective_replay_readiness_hotspot_checkpoint_ids_by_blocker_key.get(
                effective_replay_readiness_hotspot_primary_blocker_key_by_section[section],
                [],
            )
        ]
        for section in hotspot_sections
    ...
    effective_replay_readiness_hotspot_primary_blocker_next_step_by_section = {
        ...
        for section in hotspot_sections
        if effective_replay_readiness_hotspot_primary_blocker_key_by_section[section]
    ...
    effective_replay_readiness_hotspot_primary_blocker = {
        "key": effective_replay_readiness_hotspot_primary_blocker_key,
        "count": effective_replay_readiness_hotspot_primary_blocker_count,
        "checkpoint_ids": list(effective_replay_readiness_hotspot_primary_blocker_checkpoint_ids),
        "checkpoint_count": effective_replay_readiness_hotspot_primary_blocker_checkpoint_count,
        "checkpoint_share": effective_replay_readiness_hotspot_primary_blocker_checkpoint_share,
        "next_step": effective_replay_readiness_hotspot_next_step_by_blocker_key.get(
            effective_replay_readiness_hotspot_primary_blocker_key
        ),
    }
    effective_replay_readiness_hotspot_primary_blocker_by_section = {
        section: {
            "key": effective_replay_readiness_hotspot_primary_blocker_key_by_section[section],
            "checkpoint_ids": list(effective_replay_readiness_hotspot_primary_blocker_checkpoint_ids_by_section[section]),
            "checkpoint_count": len(effective_replay_readiness_hotspot_primary_blocker_checkpoint_ids_by_section[section]),
            "checkpoint_share": (
                round(
                    len(effective_replay_readiness_hotspot_primary_blocker_checkpoint_ids_by_section[section])
                    / checkpoint_section_counts[section],
                    4,
                )
                if checkpoint_section_counts[section]
                else 0.0
            ),
            "next_step": effective_replay_readiness_hotspot_primary_blocker_next_step_by_section.get(section),
        }
        for section in hotspot_sections
    }
    ...
    effective_replay_readiness_hotspot_primary_blocker_summary = (
        f"{effective_replay_readiness_hotspot_primary_blocker_key}: "
        + ", ".join(effective_replay_readiness_hotspot_primary_blocker_checkpoint_ids)
        + (
            f" ({effective_replay_readiness_hotspot_primary_blocker_checkpoint_share * 100:.2f}% of hotspot checkpoints)"
            if effective_replay_readiness_hotspot_primary_blocker_checkpoint_share is not None
            else ""
        )
        if effective_replay_readiness_hotspot_primary_blocker_key is not None
        and effective_replay_readiness_hotspot_primary_blocker_checkpoint_ids
    ...
    effective_replay_readiness_hotspot_primary_blocker_repair_summary = (
        effective_replay_readiness_hotspot_primary_blocker_summary
        + " -> "
        + effective_replay_readiness_hotspot_next_step
        if effective_replay_readiness_hotspot_primary_blocker_summary is not None
        and effective_replay_readiness_hotspot_next_step is not None
        else effective_replay_readiness_hotspot_primary_blocker_summary
    )
    ...
    effective_replay_readiness_section_priority = [
        section
        for section, count in sorted(
            effective_replay_readiness_blocker_count_by_section.items(),
            key=lambda item: (-item[1], item[0]),
        )
        if count > 0
    ]
    effective_replay_readiness_next_steps_by_priority = [
        effective_replay_readiness_hotspot_next_step_by_section[section]
    ...
    effective_replay_readiness_hotspot_tied_sections = [
        summary["section"]
        for summary in effective_replay_readiness_hotspot_summaries
    ]
    ...
    return {
        ...
        "effective_replay_readiness_hotspot_primary_blocker_checkpoint_share": effective_replay_readiness_hotspot_primary_blocker_checkpoint_share,
        "effective_replay_readiness_hotspot_next_step_by_blocker_key": effective_replay_readiness_hotspot_next_step_by_blocker_key,
        "effective_replay_readiness_hotspot_next_step_by_section": effective_replay_readiness_hotspot_next_step_by_section,
        "effective_replay_readiness_hotspot_primary_blocker_key_by_section": effective_replay_readiness_hotspot_primary_blocker_key_by_section,
        "effective_replay_readiness_hotspot_primary_blocker_checkpoint_ids_by_section": effective_replay_readiness_hotspot_primary_blocker_checkpoint_ids_by_section,
        "effective_replay_readiness_hotspot_primary_blocker_next_step_by_section": effective_replay_readiness_hotspot_primary_blocker_next_step_by_section,
        "effective_replay_readiness_hotspot_primary_blocker": effective_replay_readiness_hotspot_primary_blocker,
        ...
        "effective_replay_readiness_hotspot_primary_blocker_repair_summary": effective_replay_readiness_hotspot_primary_blocker_repair_summary,
