...
- `effective_replay_readiness_hotspot_section`, `effective_replay_readiness_hotspot_sections`, and `effective_replay_readiness_hotspot_count` expose the section with the heaviest blocker load (with tie awareness), so Playwright-style rerun recovery can start with the noisiest band first instead of re-sorting per-section counts downstream.
