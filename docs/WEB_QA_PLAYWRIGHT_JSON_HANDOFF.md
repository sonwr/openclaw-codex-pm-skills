...
- `next_action_replay_support_dimensions_present`
- `next_action_replay_support_missing_dimensions`
...
- `effective_replay_readiness_hotspot_checkpoint_ids_by_blocker_key` and `effective_replay_readiness_hotspot_checkpoint_count_by_blocker_key` turn that hotspot view into direct blocker-key -> checkpoint lanes/counts, so rerun automation can open the exact `missing_target_refs` or `incomplete_evidence_refs` repair band without re-deriving section membership first.
- `effective_replay_readiness_hotspot_primary_blocker_key`, `effective_replay_readiness_hotspot_primary_blocker_count`, `effective_replay_readiness_hotspot_primary_blocker_checkpoint_share`, and `effective_replay_readiness_hotspot_next_step`, `effective_replay_readiness_hotspot_primary_blocker_summary` condense that same hotspot signal into one deterministic repair theme, quantify how much of the hotspot checkpoint lane it covers, and provide a copy/paste rerun sentence, so automation can open with a stable first fix instead of re-ranking blocker keys itself.
- `effective_replay_readiness_hotspot_primary_blocker_key_by_section`, `effective_replay_readiness_hotspot_primary_blocker_checkpoint_ids_by_section`, and `effective_replay_readiness_hotspot_primary_blocker_next_step_by_section` mirror that deterministic first-fix lane per hotspot section, so a replay bot can pick the first blocker for `functional` / `visual` / `off_happy` without flattening tied sections back into one global queue.
- `effective_replay_readiness_section_priority_summary` compresses the blocked section order into copy-ready `section: blocker -> checkpoints` sentences, so human or bot reruns can paste the first repair lane without joining section priority and blocker summaries downstream.
- Within each hotspot summary, `blocker_counts` turns the blocker-key list into a deterministic per-section map (for example `{"missing_target_refs": 1, "missing_timestamps": 1}`), which lets CI or a replay bot branch on the dominant recovery problem without re-counting the markdown-derived keys.
- `has_next_action` plus `next_action_text` separate missing closure from a present-but-incomplete handoff note.
- `next_action_failed_check_gap_count`, `next_action_failed_check_gap_count_by_classification`, and `next_action_failed_check_gap_rate_by_classification` expose the unresolved failed-check handoff gap directly, so downstream replay automation can queue leftover selector/runtime/product work without diffing `failed_check_ids` against `next_action_failed_check_refs` again.
- `unresolved_failed_check_first_id`, `unresolved_failed_check_first_classification`, `unresolved_failed_check_first_recovery_owner`, and `unresolved_failed_check_next_step` condense the first missing handoff item into one deterministic repair cue, so a replay bot can open with a copy/pasteable next step instead of composing its own recovery sentence.
- `failed_check_handoff_summary_by_recovery_owner`, `next_action_failed_check_handoff_summary_by_recovery_owner`, and `unresolved_failed_check_handoff_summary_by_recovery_owner` collapse each owner lane into a compact `owner -> F2, V1` style map, so rerun routing can paste the per-owner queue without rebuilding grouped arrays downstream.
